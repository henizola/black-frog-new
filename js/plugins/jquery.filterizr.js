...
(function (global, $) {
  ...
  $.fn.filterizr = function () {
    ...
    if (!self._fltr) {
      self._fltr = $.fn.filterizr.prototype.init(
        self.selector,
        typeof args[0] === "object" ? args[0] : undefined
      );
    }
  ...
  $.fn.filterizr.prototype = {
    ...
    init: function (selector, options) {
      ...
      //Private properties
      self.css({
        //Cache reference to container as jQuery obj and init its CSS
        padding: 0,
        position: "relative",
      });
      ...
      self._uID = "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(
        /[xy]/g,
        function (c) {
    ...
    search: function (text) {
      ...
      if (self._isSearchActivated()) {
        for (i = 0; i < array.length; i++) {
          ...
          var containsText =
            array[i].text().toLowerCase().indexOf(text.toLowerCase()) > -1;
          if (containsText) {
            target.push(array[i]);
          }
        }
      ...
      for (i = 0; i < self._mainArray.length; i++) {
        ...
        if (typeof self._mainArray[i]._category === "object") {
          ...
          for (var x = 0; x < length; x++) {
            subArrays[self._mainArray[i]._category[x] - 1].push(
              self._mainArray[i]
            );
          }
        }
  ...
  var FiltrItemProto = {
    ...
    _init: function (index, parent) {
      ...
      self.css(parent.options.filterOutCss).css({
        ...
        transition:
          "all " +
          parent.options.animationDuration +
          "s " +
          parent.options.easing +
          " " +
          self._calcDelay() +
          "ms",
      });
    ...
    _onTransitionEnd: function () {
      ...
      if (self._parent._isAnimating) {
        ...
        else self._parent.trigger("filteringEnd");
    ...
    _filterOut: function () {
      var self = this,
        filterOutCss = self._parent._makeDeepCopy(
          self._parent.options.filterOutCss
        );
    ...
    _filterIn: function (targetPos) {
      var self = this,
        filterInCss = self._parent._makeDeepCopy(
          self._parent.options.filterInCss
        );
      ...
      filterInCss.transform +=
        " translate3d(" + targetPos.left + "px," + targetPos.top + "px, 0)";
      //Play animation
